...
// findMaxNumber(1, 5, 8, 3);
        ...
        //ЗАМЫКАНИЕ И ЛЕКСИЧЕСКОЕ ОКРУЖЕНИЕ ФУНКЦИЙ

// Lexical environment - внутренний тех объект. В котором представлена каждая функция как объект, а ее переменные - как свойства этого объекта local
// Замыкание - если функция не нашла переменную внутри себя - она пошла наверх искать

// По факту - замыкание = функция возвращающая другую функцию. Тем самым функция замыкается на область видимости родительской функции
...
function counter() {
  ...
  return function() { //функция замкнута на область видимости counter()
    count++;
    return count;
  };
};

let firstCount = counter(); //присваиваем значение count в переменную которая видна только функции с текущей областью видимости
console.log(firstCount()); // 1 вызываем внутреннюю функцию 
console.log(firstCount()); // 2 тк текущая внутренняя функция замкнута на область видимости своего родителя - то переменная count будет работать только для нее

let secondCount = counter(); //помещаем значение count в новую переменную и создаем новое локальное окружение а следовательно и счетчик создастся заново И они будут работать независимо
console.log(secondCount()); //1
console.log(secondCount()); //2
console.log(secondCount()); //3
